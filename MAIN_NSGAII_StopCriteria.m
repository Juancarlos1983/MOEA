...
%     FO = @(x) FO_TANK3hidro(x);      % Function Objective   
    ...
    sigma=0.1*(VarMax-VarMin);  % Tamanho do passo de mutação
...
%     end
%     
%     save(sprintf('população_goias_TANK4hidro_%d.mat',arquivo),'pop')
    
    %%
    population = load (sprintf('população_goias_TANK4_%d.mat',arquivo));
    for i=1:nPop
...
%             Melhores(i,:) = [F1(i).Position (F1(i,1).Cost(1)) (F1(i,1).Cost(2)) sqrt((F1(i,1).Cost(1)-F1min)^2 + (F1(i,1).Cost(2)-F2min)^2)];
        ...
        if Count > Count_Max && primeira == 0
            ...
            xlswrite(sprintf('pareto goias NSGAII_3CP+TANK4_T2.xlsx'),Melhores1,num2str(['CP1_' num2str([arquivo it])]))            
            primeira = 1;
            %break;
            t(1,arquivo) = toc;
        end
        ...
        if GD < 0.15
            if abs(GD0 - GD) < 0.001 && segunda == 0;
                ...
                xlswrite(sprintf('pareto goias NSGAII_3CP+TANK4_T2.xlsx'),Melhores2,num2str(['CP2_' num2str([arquivo it])]))
                segunda = 1;
                %break;
                t(2,arquivo) = toc;
            else
        ...
        if it == MaxIt
            ...
            xlswrite(sprintf('pareto goias NSGAII_3CP+TANK4_T2.xlsx'),Melhores3,num2str(['CP3_' num2str([arquivo it])]))
            xlswrite(sprintf('Medidas goias NSGAII+TANK4_T2.xlsx'),StopCrit,num2str([arquivo it]),'A1')
            t(3,arquivo) = toc;
            break;
        end
    end
end
save ('tempo 3C_goias_NSGAII_TANK4_T2.mat','t');